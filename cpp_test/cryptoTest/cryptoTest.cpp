...
std::string checkRsaSignature(const std::string& pubfilename, const std::string& signature, const std::string& msg) {
    ...
    try {
        StringSource ss1(decoded, true, new SignatureVerificationFilter(
...
int main() {
    /* RC4 test
    std::string plain, recover, decode,seedkey;
    std::cin >> plain;

    seedkey = randomARC4key(10);
    std::cout << seedkey << std::endl;
    decode = encryRC4(plain, seedkey, 10);

    std::cout << decode << std::endl;
    recover = decryRC4(decode, seedkey, 10);
    ...
    */
    ...
    std::string signature;
    std::string msg = "baka";
    //std::string prikfn = "prikey";
    //std::string pubkfn = "pubkey";
    //std::string sigkfn = "sigfn";
    std::string recover;
    //
    std::string pubkfn, prikfn;
    std::cin >> pubkfn >> prikfn;
    getRsaKey(pubkfn, prikfn);
    //
    signature=getRsaSignature(prikfn, msg);
    recover=checkRsaSignature(pubkfn, signature, msg);
    std::cout << signature << std::endl;
    std::cout << recover << std::endl;
   
    /*
    std::string plain, recover, cipher, pubfile, prifile;
    std::cin >> plain;
    std::cin >> pubfile;
    std::cin >> prifile;
    getRSAOAEPkey(pubfile, prifile);
    cipher = encryRSAOAEP(pubfile, plain);
    std::cout << cipher << std::endl;
    recover = decryRSAOAEP(prifile, cipher);
    std::cout << recover << std::endl;
    */
